...
module Tools
  def self.in_project(command, folder = nil, action = nil)
    ...
    system "cd ../#{opts[:proj_url]}#{folder} && #{command}"
  end

  def self.rename_placeholders_by_directory(directory, from, to, from_to)
    Find.find("../#{opts[:proj_url]}/#{directory}") do |file_name|
...
OptionParser.new do |parser|
  ...
  parser.on('-m', '--mail-service=SERVICE', 'transactional email service can be: "mailgun", "sendgrid" (defaults to: mailgun)') do |v|
    case opts[:ms] = v
    when 'mailgun'
      opts[:mss] = 'mg.'.freeze # mailservice subdomain
    when 'sendgrid'
      opts[:mss] = 'sg.'.freeze
    else
...
# copy over README
if File.exists?('readme_template.md')
  puts '** copying readme over'
  f = File.new("../#{opts[:proj_url]}/README.md", 'w+')
  f.write(File.read('readme_template.md'))
  f.close
end
...
Tools.rename_placeholders_by_file("#{opts[:proj_url]}/trellis/group_vars/all/mail.yml", nil, nil,
...
Tools.rename_placeholders_by_file("#{opts[:proj_url]}/trellis/hosts/staging",
                                  /your_server_hostname/, "#{opts[:proj]}.#{opts[:org_url]}")

Tools.rename_placeholders_by_file("#{opts[:proj_url]}/trellis/hosts/production",
                                  /your_server_hostname/, opts[:proj_url])

if opts[:acf].present?
  puts '** Adding ACF Key'
  File.open("../#{opts[:proj_url]}/trellis/group_vars/all/vault.yml", 'a') do |f|
    f << "acf_pro_key: #{opts[:acf]}\n"
  end
  File.open("../#{opts[:proj_url]}/trellis/group_vars/staging/vault.yml", 'a') do |f|
    f << "      acf_pro_key: #{opts[:acf]}\n"
  end
  File.open("../#{opts[:proj_url]}/trellis/group_vars/production/vault.yml", 'a') do |f|
    f << "      acf_pro_key: #{opts[:acf]}\n"
  end
end
...
# create .vault_pass
f = File.new("../#{opts[:proj_url]}/trellis/.vault_pass", 'w+')
